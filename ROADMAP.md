...
|---|---|---|---|
| ✅ | ~~Phase B1b: delete the tertiary machinery and collapse the value window~~ | patch 108 | **A3 is delivered in full here and not before, and the honest headline is that it moves no node anywhere.** Two variables, two commits, so attribution is exact. **(1) THE TERTIARY WAS NEVER A THIRD LEVEL.** It was read in seven places and every one read it as `primary + tertiary` -- never alone, never against a threshold of its own. It was a redundant decomposition of ONE coefficient: what a trick to the nil bidder is worth. Folded into `primary` (live/max `k*k + 1`, live/min `k*k`, dead 0), which is arithmetically the identity. **All nine baselines bit-identical**, as predicted before running. Two gates lost a conjunct and both were tautologies under the fold: `tertiary >= 0` was always true (the level was 0 or 1), and `tertiary == 0` in `value_is_nil_tricks` is subsumed by `primary == 1` -- a `(0, *, 1)` weight set never existed and live/max at `k = 1` folds to 2, not 1. **(2) K IS A SEPARATOR, NOT A UNIT.** It spaces the trick term far enough apart that a run of tricks cannot outweigh the level above it. With a dead nil that level is zero, so there is nothing to separate and `step` collapses to 1. Dead-nil weights go `(0, +/-k, 0)` -> `(0, +/-1)`. **Support 14 both directions (unchanged); window 183 -> 14 integers; density 7.7% -> 100%.** B1a pinned the 183 on purpose so this had a number to move, and that assertion is updated rather than deleted. **ALL NINE BASELINES ARE BIT-IDENTICAL AFTER (2) AS WELL, WHICH IS STRONGER THAN PREDICTED AND IS THE RESULT WORTH READING.** The prediction was that `positions.txt` full and `large.txt` full would move on their dead-nil rows. They did not, and the mechanism is exact: for a dead nil the value is `secondary * side_tricks` and nothing else, so collapsing the step divides EVERY value, alpha, beta and stored bound by exactly `k`. Every comparison in the search comes out the same way and the tree is identical node for node. **Nothing in the dead-nil path carries a constant that fails to scale** -- and in particular `presolve_eligible` is gated on `!roles.nil_already_set()`, so this shape has no root window at all today. **THIS IS A REPRESENTATION PRECONDITION, NOT A PERFORMANCE OPTIMISATION, and the entry records it that way rather than hunting for a benchmark win to report.** A denser value space with no window to spend it in buys nothing today; what it buys is the precondition. Phase D's pinned engine is what puts a window on this space, and A3's ceiling is what that window will then be worth. Anyone re-reading this for a speed number should stop here: there isn't one, and the absence is the correct result rather than a measurement that failed. **Live nil is the control and is untouched** -- support still 105, still k-spaced, pinned in the same test, because a change that collapsed `k` everywhere would pass every dead-nil assertion above and move every single-nil count in the repo. `nil_oracle.py` forked in the same commit per the plan's B1 rule, with `objective_weights` returning a 2-tuple; `mask_determinacy.py` and `make_multinil_corpus.py` updated with it. **A HARNESS MISTAKE MADE, CAUGHT AND RE-RUN, recorded because the catch is the point.** Midway through, a `git stash` was taken before a `git checkout HEAD~1` comparison build and never popped, so the working tree silently reverted to step 1. The "mutation check" that followed edited a line that did not exist in that tree, built nothing new, and **reported a pass** -- a check exercising nothing, which is this file's recurring bug arriving in the measurement harness rather than in the code. It was caught because the oracle returned `(0, -5)` where the collapse should have made it `(0, -1)`: an INDEPENDENT reading of the same quantity disagreed with the one the harness was reporting. Everything in this entry was then re-measured from `rm -rf build`. The rule this reinforces is already on the learnings list -- validate the harness against a known-good control before trusting it on the disputed case -- and the specific lesson to add is that a version-control operation inside a measurement sequence is part of the harness and needs the same suspicion as the code under test. **Mutation-checked, after the re-run**: reverting the collapse fails three assertions. Multi-nil's all-bids-down branch has the same zero level above the trick term and was deliberately NOT collapsed -- one variable per patch, and `multinil.txt` would move. **ALSO CORRECTED, IN THE PUBLIC HEADER, AND IT IS THE CORPUS CONTRACT REACHING THE API BOUNDARY:** `nil_solver.h` told callers that `nil_side_tricks - nil_tricks` is the partner count the pair can GUARANTEE. That was true of the three-level objective and is false now -- the split is a WITNESS, it moves with move ordering and with which suit is which, and `nil_side_tricks` is the pinned quantity. Comment-only, no ABI change: `ObjectiveWeights` is internal C++, absent from the public header, never positionally initialised anywhere, and unreferenced from `csharp/`, so the struct losing a field cannot reach a P/Invoke caller. A C# caller that had been reading the split was reading something legal, optimal and not reproducible. 37/37 |
| ✅ | ~~`rank_band()` crashed on any layout without a bid on each side~~ | patch 107 | **A refusal, not a tolerant default, and the tolerant default is why.** `--reachable-outcomes` died with `StopIteration` on `3 2 3 1`: `rank_band()` assumed a bidder on both the N/S and E/W axes. `side_rank(mine_makes, theirs_makes, partner_role)` is defined over BOTH sides' bids, so a layout with one bid per side is the only one that HAS a rank band. **What a tolerant lookup would have returned, measured rather than argued: (3, 3)** -- the tightest band there is, reading as *N/S keeps its bid and E/W's dies*, on a layout where N/S has no bid at all and the only bidder is already declared down. A caller would have spent it as a search window and cut the true answer out. That is the parity bug's failure mode at a different site: an assumption about WHERE A BIDDER SITS, held silently, answering a question the layout does not pose. Refused via `role_shape`, mirroring `solve_conjunction`'s existing guard. **This catches two shapes, not one, which is why it asks the shape rather than counting bidders**: `0 3 0 3` has TWO bidders and still no band, since both sit on the same axis, and a count would have waved it through and crashed exactly as before. **THE REFUSAL IS THE API CONTRACT, and the renderers are presentation on top of it.** `rank_band()` raises; there is no default, no sentinel, no `(0, 0)` and no widened band standing in for *not applicable*, and the docstring says so in a `Raises:` section rather than leaving it to be inferred. `band_is_defined()` is the sanctioned way to ASK without catching. `describe()` marks the band `n/a` with the shape named rather than dropping the line, and `--compact` omits `rank_lo`/`rank_hi` rather than emitting a placeholder a parser would read as zero -- but both consult `band_is_defined()` and neither softens the method, so a future renderer wanting a value here has to change the objective rather than this refusal. **The probe itself keeps answering on both shapes.** `reachable`, `undecided` and `name()` are defined for any number of bidders and `solve_reachable_outcomes` never consults the shape, so only the band is missing. **ALSO: `nil_oracle.py --selftest` WAS WIRED INTO NOTHING** -- not ctest, not either build script -- so the regression below would never have run. Added as ctest `oracle_selftest`, 7.6 s. A check that quietly stops checking is this repo's recurring bug; a check that was never wired up has simply never started. Mutation-checked both ways: removing the guard fails 4, and the tolerant default fails 4. 37/37 (36 + the new one); all 9 banked baselines unmoved -- the C++ binary is untouched |
| ✅ | ~~Phase B1a: the tertiary level is a fact about the SHAPE, not the DIRECTION~~ | patch 106 | **One line in `objective_weights`, and it cost four separate investigations to land honestly.** `w.tertiary = opts.minimise_own_tricks ? 0 : 1` welded two independent things: WHETHER A THIRD LEVEL EXISTS and WHICH WAY THE SECOND ONE RUNS. That weld is why the maximising direction could not reach the team-count value space the minimising one has always produced -- asking for `max` silently switched the third level back on. Now gated on the shape alone: `roles.nil_already_set() ? 0 : (opts.minimise_own_tricks ? 0 : 1)`. **LIVE NIL IS BIT-IDENTICAL IN BOTH DIRECTIONS** -- the tertiary is the `+1` in a live nil trick's `K*K + 1 - K`, and moving it would shift every single-nil count in the repo for reasons that have nothing to do with team-count. **WHAT A3 PREDICTED AND WHAT THIS DELIVERS ARE NOT THE SAME THING, and the test says so.** A3 predicted 14 values in a 14-integer window. Measured on both directions at 13 cards: **support 14 (predicted 14, MATCHES); window 183 (predicted 14, DOES NOT)**. The values are `k` apart because `secondary` is still `+/-k`, and `k` exists to separate the levels ABOVE the trick term; with primary and tertiary both zero there is nothing left to separate, so `k` can collapse to 1 and the window with it -- but only once the machinery reading those levels is gone. `(0, -1, 0)` gives 14 values in 14 integers, 100% dense. **That is the deletion patch's to deliver, and 183 is pinned in the test so the deletion has a number to move.** A3 is NOT fully realised yet and should not be recorded as such. **The symmetry is what the test pins, not the new direction.** Checking only that `--secondary max` returns the team count would pass just as well if someone re-welded it the other way round, so both directions are pinned against the same 14-value support, with `max`'s support asserted to be exactly the negation of `min`'s. 26 checks. **BASELINE MOVEMENT, LOCALISED AND PREDICTED BEFORE IT WAS MEASURED:** the dead-nil/max rows were counted first, then the benchmark run. `positions.txt` full **278,059 -> 274,270 (-1.36%)**, caused by the dead-nil/max canonicalisation change across its **57** such rows; `large.txt` full **163,149,275 -> 163,134,302 (-0.01%)**, from its **four** (`c7-0003`, `c7-0005`, `c8-0001`, `c8-0002`). The other seven banked baselines are unmoved: `fast` mode is `(1,0,0)` regardless of everything, and the multi-nil and opposed branches already set tertiary zero. **Every baseline that moved contains dead-nil/max rows; every one that did not, does not.** **THE ELEVEN ROWS WERE NOT RE-BANKED, AND THAT IS THE REAL RESULT OF THIS PATCH.** 11 rows of `positions.txt` changed their `nil_tricks`; `side_tricks` -- the team total -- is identical on all eleven. The re-bank was written, and `tools/invariants.py` then **rejected it**: permuting suits on `c4-0032` moved the split 2 -> 3. **The split is not a fixed point.** Canonicalisation derives the line from move ordering, move ordering depends on suit identity, so once the tertiary stops constraining the split it becomes an artifact of which suit is which -- and of the transposition table's contents, and of the search window, which is why eight corpus variants disagreed about it and **none of them disagreed about a total**. The re-bank was reverted. **`nil_tricks` and `pv` are now NON-AUTHORITATIVE on dead-nil rows**: the columns stay populated (an 11-row semantic change is not a 560-row schema change) and the line is still checked for legality and optimality, but nil_bench no longer compares them for equality there, and `invariants.py`'s `undetermined_split` -- which already existed for the `min` direction, gated on `secondary == "min"` -- now covers `max` too. **Regression, on the correct symmetry group:** spades is trump, so only the three side suits may be relabelled; over all six such relabellings `side_tricks` is INVARIANT on 11/11 while the split moves on 6/11. **An earlier version of that check permuted all four suits, which moves the trump suit and is not a relabelling at all** -- it reported side_tricks varying on 11/11 and was wrong. **What this cost and what it bought:** the claim in this entry's first draft that the oracle and solver agree on "11/11 canonical splits" was over-read. They do agree, but as two programs that order moves similarly on these inputs -- not about a quantity the objective determines. The team-total agreement is the real result and it stands |
| ⚠️ | The oracle hardcoded which pair minimizes, and it had been wrong for every nil bidder sitting at E or W | separate patch | **Found while trying to validate an 11-row re-bank, and it is the reason that re-bank was not taken on the solver's word.** `nil_oracle.py`'s single-nil `_search` read `maximizing = (seat % 2) == 1  # E and W`, hardcoding the minimizing side to parity 0 whatever seat had actually bid. **Two lines below, the SCORING derives the pair correctly** from `ctx.designated % 2` -- its own comment reads "whichever it is" -- and `_search_multi` has always derived it too (`!= ctx.minimizing_parity`). Only this line did not. **What it cost:** with the bidder at E or W the value accumulated for the E/W pair while E/W were told to MAXIMISE it, so both sides played the wrong way round and the opponents cooperated with the nil pair. **The error was one-directional** -- the reported pair total was never below the true one and usually above it -- **which is exactly why it survived: it returned a plausible answer rather than crashing.** Evidence, six dead-nil corpus rows pinned by the solver and unchanged since they were banked: three with the bidder at W were WRONG before and correct after; two with the bidder at S are the positive control and did not move; one has the bidder at W but a pair total of zero, where both assignments coincide. **3 fixed, 0 regressed, 2 positive controls held.** `c5-0062` (bidder at E) confirmed it out of sample. Pinned as a `--selftest` regression block, checked as `(tricks, side_tricks)` rather than as the split alone -- **the bug moved the PAIR TOTAL, so a split-only check would have passed throughout.** **A FALSE STEP WORTH RECORDING.** The first attempt to test the parity hypothesis re-expressed a deal with a `W:` PBN anchor so the bidder would land at index 0. Both tools returned identical answers and it looked like a refutation. It was not a test at all: `parse_pbn` indexes hands by ABSOLUTE seat, so the anchor could never have changed what it was meant to change. A hypothesis that has only been tested by a void experiment is untested, not refuted |
| ⚠️ | The oracle carried the same tertiary weld, and two of its own selftests encoded it | separate patch | **Mirrored from patch 106 so the two objectives stay comparable: an oracle answering a different question than the solver is worse than no oracle.** `tertiary = 1 if secondary == "max" else 0` becomes `0 if nil_already_set else (...)`. Landed AFTER B1a and separately from it, because the eleven rows could not be adjudicated while one side had the weld and the other did not -- the two would have agreed on the team total and disagreed on the split by construction, and the split is half of what the re-bank needed. **Applying it surfaced two selftest assertions that encoded the weld rather than tested it:** `objective_weights(4, "max", True) == (0, -5, 1)`, and the oracle's own `nil set: and its partner ends up holding it`. Both are the mirror of the assertion patch 106 had to rewrite in the solver's tests, for the mirror of the reason. **Still outstanding and deliberately not touched here: `rank_band()`** at line 1373 assumes the nil bidder sits at `t` or `t+2` and raises `StopIteration` on a `3 2 3 1` layout, which is what `--reachable-outcomes` crashes on. Same underlying assumption about where a bidder sits as the parity bug, different site, its own patch. It removes the tool that would otherwise adjudicate a disputed outcome directly, so it is worth fixing before the next such dispute rather than during one |
| ✅ | ~~Phase A: the four measurements that gate the rearchitecture~~ | patch 105 | **Four measurements, no solver code, and two of them cancel work that was already scoped.** The rearchitecture plan ordered Phase A first precisely so that A1-A4 could kill work before it was written; A2 and A4 did. **A4 -- CAN THE COOPERATIVE PROBE FIND DOWNGRADES AT 13 CARDS? It can answer, and there is nothing to find.** On `nil13_verdict.txt`: 400 rows, 250 with the nil forced set, 89 of those settled free by `nil_must_take_a_trick`, 161 probed at an 8M budget -- **0 unreachable, 146 reachable, 15 exhausted**, 180,785,173 nodes, 106.5s. The exit condition this was written for was *most exhaust*; only 9.3% did, and **it failed for a stronger reason than the one anticipated**. A nil forced set adversarially is almost always still alive on SOME cooperative line, so the probe returns *reachable* and the 1 -> 3 downgrade is not licensed. Outside the static proofs the probe found **zero** additional downgrades on this corpus. That is not slowness; there is nothing there. **A2 -- IS PLAIN TRICK MAXIMISATION CHEAPER OR DEARER? Dearer, on exactly the population the downgrade fires on.** Arm A `2 3 0 3` (live nil, today's full mode) against arm B `2 3 1 3` (nil declared down -> primary 0 -> plain trick maximisation), same cards, same leader, one process per solve. On all **89 statically-doomed deals**, 88 paired at a uniform 280s cap: **median 1.83x nodes, geometric mean 1.74x, aggregate 7,212,816,411 -> 12,765,796,137 = 1.77x**, wall 856s -> 1474s = 1.72x. **Throughput is flat** (8.42M against 8.66M nodes/sec), so the node ratio is the real ratio and this is not items 44 / C1 / C3's failure wearing a new hat. B is worse on 66 of 88 (75%), >=2x on 40 (45%), >=4x on 14 (16%), worst `v13-0328` at **10.61x**; B is CHEAPER on 22 of 88 (25%), best `v13-0181` at 0.26x. **The mechanism is visible in the output rather than inferred: arm B reports `presolve_nodes=0`**, because `presolve_eligible` is gated on `!roles.nil_already_set()`, and `nil_must_take_a_trick` is guarded off because `per_nil` goes negative. The shape runs with almost none of the machinery the live-nil question gets. **A SAMPLING ERROR CAUGHT AND CORRECTED MID-MEASUREMENT, and it is the reason the first number was wrong by 4.6x.** The first deal measured, `v13-0000`, came back at 8.38x -- and it carries `nils_set=0`, so the nil MAKES and it is not in the downgrade population at all. A stratified 25-deal sample of genuinely doomed deals then read 1.25x median, and the correct 89-deal population reads 1.83x. **Three different answers from three different populations on the same question**, which is patch 69's lesson and patch 83's in one measurement. **A3 -- DOES THE VALUE SUPPORT COLLAPSE? Yes, 7.5x, and the new shape already exists in the binary.** Today at `k = t + 1 = 14`: a nil trick is worth `k*k + 1 - k = 183`, a cover trick `-k = -14`, so the support is the `(n,p)` pairs with `n + p <= 13` -- **105 values, distinct because `gcd(183,14) = 1`** -- spread over `[-182, 2379]`, **2,562 integers at 4.1% density**, with the presolve seeding either `beta <= 1` (183 integers wide) or `alpha >= 15` (2,365 wide). Under the new team-count rule the value is `S = n + p` in `[0,13]`: **14 values, a 14-integer window, 100% dense.** `--seats 2 3 1 3 --secondary min` already yields `value = 14 * S` on this binary -- primary 0 AND tertiary 0 -- so the collapse is not speculative. The maximising direction is unreachable only because `tertiary = minimise_own_tricks ? 0 : 1` welds *no tertiary* to *min*; that weld is the work. **A1 -- WHAT DOES THE `satisfied` BIT COST? About one node in thirty, AND THE MECHANISM IN THE PLAN WAS WRONG.** Measured with a throwaway instrumented build (NOT shipped -- see below) that appends one state-derived bit to the transposition key and changes nothing else: 24 of 25 paired, **median 1.035x nodes, aggregate +9.4%, stores median 1.039x, wall median 1.033x**. The plan said the bit *widens the key and lowers the hit rate on every node*. **It does neither.** Hit-rate delta is **+0.01pp median** across the sample (range -0.13 to +0.25pp) -- flat to three significant figures -- while stores (+3.9%) and nodes (+3.5%) track each other almost exactly, which is the signature of MORE DISTINCT ENTRIES rather than worse lookups. Key width is not binding either: the key is `21 + 2n` bits at a trick boundary, so 13 cards goes **125 -> 126**, inside the 128-bit budget. The four-bit `carry_nils_broken` field costs the root position; one bit costs nothing. **The cost also concentrates where the table is worst**: `v13-0354` moved -0.0% at a 90.7% hit rate (a table that good has nothing left to split) against `v13-0316` at +15.1% on 452M nodes and 84.6%. **THIS IS A PROXY AND THE ENTRY SAYS SO RATHER THAN BURYING IT.** It prices the KEYING MECHANISM on the live-nil tree; the merge rate it splits is this tree's merge rate, not the pinned tree's, and it is not a measurement of the Phase D type-1 engine, which does not exist. `tt_stores` is reported beside `nodes` because it is the direct measure of how much splitting actually occurred. **NOTHING FROM A1 IS IN THIS PATCH.** A `--satisfied-bit` flag whose only consumer is an experiment is a feature with no user; the instrumentation ships as a separate reproducible artifact instead. Control arm verified bit-identical before any number was read off it -- 39,701 fast / 278,059 full reproduce exactly. **ALSO IN THIS PATCH, and it is the same class of bug twice:** the banked-baseline bookkeeping, below |
| ⚠️ | Banked baselines: one source of truth, and figures that had gone stale where no re-bank looked | patch 105 | **Patch 97 exists because a re-bank missed three workloads that are run by hand. This is that same failure found twice more, and the fix is structural rather than another round of diligence.** **(1) THE WORST-CASE FIGURES IN BOTH BENCH SCRIPTS WERE STALE.** `scripts/run-bench.{sh,cmd}` carried `c13-0000 60,020,405 / c13-0001 71,253,358 / c13-0002 32,230,695`, summing to 163,504,458. HEAD measures **59,483,222 / 70,957,819 / 32,058,737, total 162,499,778**. **That old total EXCEEDS the whole-file figure patch 95 recorded BEFORE it raised the table (163,393,676), so these had already drifted when patch 95 re-banked around them** -- at least two re-banks stale, and the solver was never wrong. **The staleness matters by the script's own standard, which is what makes it worth an entry rather than a commit message**: the closing text tells the reader that a 1% move on these rows is real and not sampling noise, and then hands over figures 0.5-0.9% high. A run that changed nothing read as a small win. **(2) EACH HAND-RUN WORKLOAD NEEDS A DIFFERENT INVOCATION AND A WRONG ONE RETURNS A PLAUSIBLE NUMBER.** The opposed corpora carry their seat roles in a header COMMENT rather than in their rows, so `nil_bench` cannot read them: `opposed13.txt` needs `--deals --seats "0 0 3 2"`, `opposed13_settled.txt` needs `--deals --seats "3 2 0 0"` (ROTATED, which is the whole point of that file), and `opposed13_real.txt` needs `--corpus` and no `--seats` at all. **None of the wrong combinations errors.** `--deals` on `opposed13.txt` with default seats returns 125,516,991; on `opposed13_settled.txt` it returns **241,340,102 -- which is patch 97's own figure for the hard single deal**, a number a reviewer would recognise and wave through. **A wrong invocation returning a FAMILIAR number is not something eyeballing catches**, which is why the fix is not to document the invocations more loudly. **`tools/check_baselines.py` holds all nine workloads, their exact invocations and their banked counts in ONE place and COMPARES rather than prints**; both bench scripts call it behind `NIL_RUN_BASELINES=1` (off by default, ~740M nodes). Verified against a deliberately corrupted `--seats`: the run reports `opposed13 125,516,991 MISMATCH, banked 351,156,828` and exits 1. All nine match on HEAD. **A ninth row, `large.txt 13c only` = 162,499,778, is NOT one of the four hand-run baselines** -- it is a consistency check on the worst-case leg's subtotal, which is the figure that went stale and which no other row covered. **Also fixed here, one word:** `run-bench.sh`'s multi-nil guard tested `-z`, so `NIL_SKIP_MULTINIL=0` SKIPPED the leg -- the opposite of its own header -- and now matches `NIL_SKIP_WORST`'s `!= "1"`. **Confirmed NOT transposed**: every `39,701 / 278,059 / 49,084 / ... / 4,833,200` list in the entries above is correct as written -- 49,084 is `large.txt` fast and 4,833,200 is `multinil.txt`. The transposition was in the working design document, not in this file, and is noted here so the next reader does not re-derive it |
| ⚠️ | `run-bench.cmd`'s two-nil leg had never executed -- the two bench scripts were running different workloads | patch 105 | **A separate bug from the baseline bookkeeping above, landed in the same patch because it is the same question -- does the thing that measures us actually run? -- and recorded on its own row so it does not get read as a comment fix.** The multi-nil leg in `scripts/run-bench.cmd` sat below `exit /b 0` AND below the `:not_built` and `:fail` labels. **Batch falls through labels and stops at `exit /b`, so control could not reach that block by any path**, with or without `NIL_SKIP_MULTINIL` set: `goto :multinil_done` was the only thing that ever jumped near it, and nothing jumped TO the leg. **So `run-bench.sh` and `run-bench.cmd` have been running different workloads** -- the `.sh` timing the six 13-card two-nil rows on every invocation, the `.cmd` silently skipping them **on the platform that is primary for this project**. Anyone benchmarking a two-nil change on Windows was measuring single-nil corpora and a worst-case leg, and would have seen a two-nil regression as no change at all. Moved above the `pause`/`exit /b 0` tail, where the `.sh` runs it, with `:not_built` and `:fail` left after the tail where they belong. **NOT VERIFIED BY EXECUTION.** There is no `cmd.exe` in the environment this was written in, so what is claimed here is a control-flow review -- every label reachable, every `goto` resolving forward to a label that exists, the two failure labels still unreachable by fall-through -- and NOT a run. Running it is T's to do locally, and this entry says so rather than implying a green tick it did not earn. The checking logic itself is in `tools/check_baselines.py` precisely so the `.cmd` calls a tested program rather than reimplementing the comparison in batch, which would have been a second unverifiable thing |
| ⊘ | Phase E's doom downgrade: retyping a proven-dead nil from type 1 to type 3 | REJECTED, patch 105 | **Rejected on availability AND on cost, before a line of it was written. A4 says it can almost never fire; A2 says the places it CAN fire are the worst places to do it.** The proposal: once a nil is known dead, stop pinning it (type 1, *must take at least one trick*) and let it maximise tricks freely (type 3), on the reasoning that the pin costs a `satisfied` bit in the transposition key for the whole search. **The trade as scoped was real and both halves of it measured the wrong way round.** The bit costs **3.5%** (A1). Not having a nil constraint to prune against costs **83%** (A2, median over the 89 deals where the downgrade could fire). And the downgrade is available on **0 of 161** deals the static proofs do not already settle (A4). **So the only cases where the downgrade is licensed are exactly the cases where it surrenders the most pruning**, and there are no others. **Type 1 is not merely the safe default under the asymmetry rule -- on this corpus it is the FASTER one**, which is a stronger claim than the design needed and comes from measurement rather than from the argument. Full write-up under *Evaluated and rejected*, including what would revive it. Consequences recorded there: `makes_cooperatively` leaves the pipeline (Q3 answered -- not on the wire, not computed, and the probe stays as callable tested code that nothing consults), `--no-doom-downgrade` has nothing left to gate, Phase E reduces to the translator plus the type-1 default, and **item H is re-justified around decision 9 rather than around Phase E** -- with the downgrade gone, what still manufactures `3 3 3 3` is every mid-hand call on an already-dead nil, and A2 measures that shape at 1.77x aggregate with no bounds at all |
| ✅ | ~~Item 60 step 6: the open cell resolved by backward induction -- the decision procedure is complete~~ | patch 104 | **The last correctness gap in item 60 is closed. Every cell of the same-lean decision procedure now agrees with the oracle's exhaustive backward induction.** The cell patch 100 left open -- both bids individually reachable AND neither side able to force rank 3 -- is resolved by `search_equilibrium` in `same_lean.cpp`: a tree walk where each mover takes the child its OWN `side_rank` prefers. **WHY A SEARCH RATHER THAN A FIFTH PROBE, which is the whole lesson of this cell.** Every probe in this codebase asks either a fully cooperative question ('does a line exist') or a one-sided forcing one ('can this side guarantee X'). This cell is neither: both sides prefer the SAME outcome -- same lean, so they agree which middle they want -- and still cannot simply take it, because the path there can pass through a node where the other side would rather defect to its rank 3. **Defection is a property of the tree, so it takes a tree walk**; patch 100 measured what ignoring that costs (cooperative reachability wrong on 48 of 116 OPPONENT-lean deals). **WHAT MADE IT TRACTABLE, and it is a real simplification rather than an approximation:** `side_rank` is a BIJECTION from the four outcomes onto {0,1,2,3} under either lean. So a mover's preference totally orders the outcomes with NO ties, and two children giving the mover equal rank give the identical outcome. The equilibrium OUTCOME is therefore well defined with no tiebreak rule at all -- which line reaches it is not, and is not reported. That is what lets this be an ordinary backward induction returning a 2-bit outcome instead of a maxn search carrying tuples whose ties would need a rule nobody has. **This was checked in seats.cpp before being relied on, not assumed from the shape of the table.** One exact early exit: once both bids are down, no later play can change the outcome, so the rest of the deal is not played out. Equivalence-collapsing is used, sound for the same reason as in `outcome_constraint.cpp` -- a relabelling permutes card identities, never which SEAT wins. Budgeted by `same_lean_probe_budget` like the probes, and exhaustion reports as exhaustion, never as an outcome. **CORRECTNESS.** `same_lean_partial_crosscheck` rewritten: the cell that used to be checked only for REFUSING is now checked against the oracle like every other cell -- the strongest check available, and warranted here because this is precisely where a plausible shortcut was already measured wrong once. Three hand sizes, three seeds, zero mismatches: 200 deals at 4 cards (8 / 107 / 33 / 52 across the four cells), 120 at 5 cards, 25 at 6 cards. Patch 100's own counterexample deal (`N:.T.KT8. T.J7.2. ..Q.A74 QJ.Q.5.`, `0 0 3 3`) now answers both-survive, matching the oracle that disproved the old rule. 36/36. **WHAT THIS DOES NOT DO: make 13-card same-lean deals answerable.** The search has no alpha-beta (the objective is not zero-sum), no transposition table beyond a local memo, and no move ordering, so at 13 cards it hits its budget. T's original failing deal now reports BUDGET EXHAUSTION rather than the open cell -- the honest state, and a different problem from the one this patch fixes. **Item 60 is now correctness-complete and performance-blocked**, and performance is the next item |
| ✅ | ~~Item 60 step 5: wiring the trick split into solve(), and budgeting the probes that made that unsafe~~ | patch 103 | **The wiring is the small half of this patch; the budgets are the half that was actually missing.** `solve()` now reports real trick counts for a same-lean deal whose outcome the decision procedure settles, instead of `TRICKS_NOT_COMPUTED`: `nil_tricks` the near bidder alone, `nil_side_tricks` the near pair, `opponent_tricks` the far pair, from step 4's `solve_constrained_tricks` under the pinned outcome. MODE_FAST still reports nothing, unchanged. **A MISTAKE CAUGHT BY MEASURING THE WIRING RATHER THAN THE THING I HAD JUST BUILT.** The first cut budgeted only the TRICK search, because that was the slow thing patch 102 had just measured. It did not help: 4 of 40 random 13-card same-lean deals still ran past 60s. The bottleneck is UPSTREAM, in the decision procedure's own `solve_cooperative` -- an OR-only search has no adversary and therefore NO CUTOFF on a negative answer, so every line must be tried and found wanting. Measured on a random 13-card deal: `--cooperative N` alone takes **67.6s**, while the other three probes on the same deal take 0.6s, 0.6s and 1.1s. Budgeting the wrong search protected nothing, and only running the wired code at 13 cards exposed it. **BOTH COOPERATIVE PROBES NOW TAKE A NODE BUDGET** (`SearchOptions::same_lean_probe_budget`, default 8M; `same_lean_trick_budget` likewise for step 4's search; 0 lifts either). This also closes the robustness gap first flagged in patch 98 and only half-fixed in patch 102, which capped MEMORY but not TIME. **EXHAUSTION IS A THIRD STATE, NOT A FALSE.** `CooperativeSolution::exhausted` / `CooperativeFailSolution::exhausted` mark a search that gave up; when set, `reachable` is MEANINGLESS rather than false. 'No line exists' and 'I stopped looking' are different facts and only the first is an answer -- every branch of the decision procedure reads those booleans, so an exhausted probe stops the procedure instead of being silently read as a negative. **AND THE REFUSAL SAYS WHICH REASON IT IS.** A first pass reported budget exhaustion through the existing open-cell message, which blames a disproved tiebreak for what is really a time limit -- misleading in exactly the way that wastes someone's afternoon. `SameLeanOutcome::exhausted` now separates them and the error names the budget, the node count reached, and the option to raise. On 20 random 13-card same-lean deals: 9 genuinely open cell, 11 budget-limited and recoverable, **0 hangs, worst case 12.8s** (before: 4 of 40 unbounded past 60s). **CORRECTNESS.** Every wired trick split checked against step 4's independent reference -- 94 of 94, both leans, including the pair-vs-seat field semantics. Separately, that a budget only ever SUPPRESSES an answer and never changes one: 200 budgeted `solve()` outcomes against the oracle's exhaustive backward induction at 5 cards, zero mismatches. No regression on mixed-lean deals -- the 13-card settled deal still returns exactly 55,428,602 nodes, matching its banked count. A node-accounting bug introduced during the wiring (the trick search's nodes overwritten rather than added) was caught and fixed. 36/36. **WHAT THIS DOES NOT DO:** make 13-card same-lean deals resolve. 11 of 20 now refuse for budget rather than hanging, which is a better failure, not a success -- the underlying searches still need the alpha-beta, transposition table and move ordering that patch 102's entry describes. The open cell itself is untouched and still needs a defection-aware criterion |
| ⚠️ | Item 60 step 4: the cover-hand trick split, correct but only to ~9 cards | patch 102 | **Implements T's priority, verified against an independent reference, and does NOT reach 13 cards. The limit is the headline, not a footnote.** `solve_constrained_tricks` in `nil/outcome_constraint.{hpp,cpp}`, `--pinned-tricks` in `nil_cli`. **THE PRIORITY, T's, and it is one idea rather than three rules:** a trick taken by a nil bidder counts toward NOBODY's bid, not even its partner's, so a trick landing on a set nil is a WASTED trick for that team. Each side therefore wants, in order: (1) maximise its own cover hand, (2) minimise the other side's cover hand, (3) maximise the other side's NIL. Level 3 is level 2 seen from the other end -- the four counts sum to the tricks remaining, so with 1 and 2 fixed, maximising their nil is identical to minimising one's OWN nil -- and the order is COMPLETE, since fixing two cover hands and one nil determines the fourth count. Level 3 is vacuous when a nil is pinned to MAKE and binds only when both are set, which is the outcome step 1's branch A produces most often. **NOT A ZERO-SUM SCALAR, measured before building.** Both cover hands can prefer the SAME outcome (each would rather take a trick itself than let a dead nil absorb it), so it cannot be one number with a sign flip, and the existing alpha-beta engine cannot carry it. Against the closest zero-sum stand-in -- 'maximise my tricks minus theirs', which WOULD drop straight into that engine -- T's order disagrees on 3 of 227 hands, and the disagreements are substantive rather than tie-breaking noise: on `N: SA H6 D5 D9 / E: C6 C7 CT CA / S: S7 H3 D2 D3 / W: D4 DJ DK CJ` with N pinned set and E pinned make, T's order gives N=1 S=1 W=2 while the stand-in gives N=3 S=0 W=1 -- the stand-in surrenders a trick of W's OWN to cost S two, widening the gap, which T's order never does. The two models I tested BEFORE T's clarification ('far minimises S' and 'far maximises its own') disagree with T's order on 38 and 46 of 227 respectively, so neither was a usable approximation either. **A SELF-INFLICTED 17x, found by measuring rather than by review.** The first draft did not collapse rank-equivalent moves, on the stated worry that cards equivalent for winning THIS trick might not be equivalent for who wins later ones. That worry is wrong: `relevant_cards` keeps exactly the cards the rest of the deal can still tell apart, so playing either of two collapsed cards leaves positions identical up to relabelling those cards -- and a relabelling permutes card IDENTITIES, never which SEAT takes a trick, which is all this search reports. Enabling it left every crosscheck split unchanged and took the worked example from 13,767 nodes to 816. **Conservatism that costs 17x needs its own justification, and this one did not have one.** **A REAL ROBUSTNESS BUG FIXED, first flagged in patch 98 and left unfixed there:** the memo had no size cap, and at 13 cards this search was OOM-KILLED by the host rather than returning anything. A library that can kill the calling process is broken regardless of its speed on hands that fit. The memo is a pure cache -- dropping an entry costs time, never accuracy -- so it now stops growing at ~4M entries (~300 MB) and the search finishes without new memoisation past that. Confirmed answer-neutral by the crosscheck. **THE LIMIT, measured with collapsing and the cap both in:** 5 cards 21k nodes; 7 cards 1.5M, 0.7s; 9 cards 3.3M, 1.6s but only 1 of 4 deals inside 25s; 10 and 11 cards 0 of 4 inside 25s; 13 cards does not finish in 240s. **So this is correct code that does not yet reach the card counts the project optimises for**, and it is marked ⚠️ rather than ✅ for that reason. What it is missing is not a constant factor: it has no alpha-beta (the objective is not zero-sum, so the existing pruning does not apply), no transposition table, and no move ordering. Closing that is its own item -- the Sturtevant/Korf reduction in the project's own reference literature is the obvious place to start, since a lexicographic order over a small outcome set may admit shallow pruning even where full alpha-beta does not. **CORRECTNESS.** `tools/constrained_tricks_crosscheck.py` (new ctest) against an independent backward induction written from the priority directly, constraint applied the slow way at the terminal state, sharing no code with the C++. Both leans checked, since a bug swapping 'my cover hand' for 'theirs' would pass a one-lean run. 120 queries at 4 cards and 72 at 5 cards across two seeds, zero mismatches, both satisfiable and unsatisfiable seen, memo ablation answer-neutral. The worked example above reproduces exactly. 36/36. **NOT IN THIS PATCH:** wiring any of this into `solve()` -- same-lean solves still report `TRICKS_NOT_COMPUTED` -- and `solve_moves()`, still refusing |
| ✅ | ~~Item 60 step 3: the pinned-outcome constraint, shipped alone~~ | patch 101 | **The FILTER half of T's framing -- "solve where nil 1 is set and nil 2 makes while maximizing tricks for the cover hands" -- with the second half deliberately left out.** New `nil/outcome_constraint.hpp`/`.cpp`, `--require-live <seats>` / `--require-set <seats>` in `nil_cli`. The constraint is T's own OR-of-discards: with nil 1 pinned set and nil 2 pinned makes, discard every line where nil 1 makes OR nil 2 is set. No objective, no trick counts, no optimisation inside it. **THE TWO HALVES ARE NOT SYMMETRIC, which is the substance of this patch.** `--require-live` (must MAKE) is checkable the instant it is violated -- a bid never un-breaks, so the subtree is abandoned WITHOUT RECURSING, which is free and is real pruning. `--require-set` (must be SET) has no cheap early failure: a seat still holding cards and not yet having won is not doomed, it may win on the last trick, so absent a structural proof this file does not attempt, the only certain failure point is the TERMINAL state. Measured on the settled 13-card deal with both bidders doomed: 12,056,872 nodes of which **9,093,017 were early must-make abandonments** -- the pruning half is carrying most of the tree. **AN INDEPENDENT CONFIRMATION WORTH RECORDING:** that 12,056,872 is the SAME node count `solve_cooperative` banks on the same deal in patch 98's entry. Two separately written searches, different files, different state representations, agreeing to the node on a 12M-node problem. **A DESIGN SKETCH FROM THE CONVERSATION IS WRONG AND IS CORRECTED HERE BEFORE STEP 4 CAN INHERIT IT.** The proposal was to mark a dead line with a sentinel value, "negative infinity for whoever is maximising". That fails: at a MINIMISING node a hugely negative value is ATTRACTIVE, so a minimiser steers INTO dead lines. Flipping the sentinel's sign per node type fails for the mirror reason once the value propagates past a node of the other type. The only sound shape -- used here and already used in `cooperative.cpp` -- is that a dead child is SKIPPED by its parent and "every child here was dead" propagates as its OWN signal, distinct from any value. `bool` is that signal in this file; step 4 needs the same separation between "no valid line" and "a valid line worth N". **CORRECTNESS.** `tools/outcome_constraint_crosscheck.py` (new ctest) against an independent reference that applies BOTH halves at the end of the line, the slow obvious way, sharing no code with the thing under test -- so what is checked is precisely the early-abandon optimisation. 120 queries over four constraint shapes (each orientation of the pinned outcome, plus each half alone so neither is only ever exercised beside the other): zero mismatches, both answers seen (75 satisfiable, 45 not), the early-abandon path fired on 75, collapsing and the memo both answer-neutral. The run FAILS if the early-abandon path never fires, so the check cannot silently stop testing its own subject. 35/35. **NOT IN THIS PATCH, and step 4 is now known to be harder than when it was scoped:** the cover-hand trick objective. A direct test found that "near maximises its cover hand's tricks, far MINIMISES them" and "near maximises its cover hand's tricks, far maximises ITS OWN cover hand's tricks" are different games -- 31 of 260 hands disagree. The doomed bidder's own tricks count toward neither side's bid, so `near_bidder + cover_near + cover_far = tricks_remaining` has a third term each side ignores, and the two sides' objectives do not sum to a constant. **That is the same non-constant-sum obstacle as the rank dimension, one level down**, and it makes step 4 a design question (which game is intended?) rather than a change of what to count. Concrete instance, 4 cards, N's nil pinned set and E's pinned make -- `N: S5 S7 DA C9 / E: S4 H4 HA DT / S: H5 HJ D5 CJ / W: SJ D2 C8 CK`: with W minimising S's tricks the split is N=2 S=0 W=2; with W maximising its own it is N=1 S=1 W=2. **W gets 2 either way** -- the difference is only whether the spare trick lands on S or on the doomed bidder, and under the first model W spends effort pushing it onto a seat whose tricks are worth nothing to anyone. Awaiting T's call on which is intended |
| ✅ | ~~Item 60 step 2: the forcing rung of the delicate cell, and the DISPROOF of the tiebreak meant to finish it~~ | patch 100 | **The rule this step set out to ship is wrong, and the measurement that killed it is the main content of this entry.** THE CLAIM, from the design conversation and stated as proved there: in the delicate cell (both bids individually cooperatively reachable) where NEITHER side can force rank 3, the equilibrium is confined to the two middle outcomes, both leans agree which middle they prefer, so the preferred one wins WHENEVER IT IS COOPERATIVELY REACHABLE. **Checked against `solve_opposing_nils` before shipping rather than after: wrong on 48 of 116 OPPONENT-lean (`3/3`) cells and 5 of 116 COVER-lean (`2/2`) cells.** Not a corner case; a wrong rule, and wrong under BOTH leans, not just the one the asymmetry argument suspected. **THE MECHANISM, traced on a concrete deal (`N:.T.KT8. T.J7.2. ..Q.A74 QJ.Q.5.`, `0 0 3 3`) rather than inferred:** both sides rank (die,die) at 2 and (live,live) at 1, and (die,die) IS cooperatively reachable -- `--cooperative-fail NE` says so -- yet the oracle's equilibrium is (live,live), ranks [1,1]. Reaching (die,die) needs TWO separate events, each bidder taking a trick. Between them the position is asymmetric: whichever bid is still alive now has rank 3 available (mine lives, theirs already dead), which beats the 2 it was cooperating toward. That side defects; the side that went first lands on rank 0. Anticipating it, neither goes first. **A cooperative probe structurally cannot see this, because cooperation is the assumption it makes** -- reachability is the wrong criterion for a goal that needs two events plus trust, and that is the general lesson, not a fact about this deal. What would revive the rung is a criterion that models DEFECTION: the question is not 'does a line to the preferred middle exist' but 'can it be reached without passing through a state where the other side prefers to flip'. **WHAT SURVIVED, each verified separately rather than inherited from the failed proof.** (a) The confinement claim holds: across 232 delicate cells the equilibrium is (live,live) or (die,die) every time, never mixed. (b) **Rung 1 is sound and is what this patch ships:** if either side can force rank 3 -- own bid alive, other's dead -- it gets exactly that; 280 cases, both leans, ZERO mismatches. (c) The conjunction gate widening is sound: `conjunction_value` is rank 3, and `side_rank` returns 3 from its first branch before it reads `partner_role` at all, the lean only ever swapping the two middle rungs. Confirmed by reading the code AND by 120 oracle deals where `0 0 3 2`, `0 0 2 2` and `0 0 3 3` give identical conjunction answers on the same cards. **THE GATE WIDENING IS DELIBERATELY NARROW.** `seat_shape(...) == SHAPE_OPPOSING_NILS` became 'either lean' for the conjunction probe ONLY. `ctx.opposing` stays false for same-lean, because it gates the rank-band machinery, which reads `far_partner_role` and genuinely is lean-dependent. Only the indicator is lean-free; only the indicator was widened. Also reordered solve()'s same-lean early-return to fire only when `conjunction_seat < 0`, or `--conjunction` on same-lean roles would be swallowed by the case-analysis path and answered with a bid mask instead of the boolean asked for. **A STEP 1 BUG FOUND BY BUILDING ON IT:** `solve_cooperative_fail` had a header declaration and an inner recursion but no public entry point -- it did not link the moment anything called it. Implemented, exposed as `--cooperative-fail <seats>`, and verified against an INDEPENDENT brute-force reference (the oracle has no fail-mode equivalent, so checking it against a cousin of itself was not an option): 120 queries, 0 mismatches, both answers seen, collapsing and the memo both answer-neutral. It is used by nothing in the shipped procedure -- the rung it was built for is the disproved one -- but it is correct code and tested as such. **NET:** the open cell is narrower, not gone: both bids individually reachable AND neither side able to force. In the shipped crosscheck 33 of 85 delicate cells now resolve and 52 refuse honestly. New ctest `cooperative_fail_crosscheck`; `same_lean_partial_crosscheck` rewritten to check all four cells including that the open one REFUSES rather than guesses. 34/34. **NOT IN THIS PATCH:** the open cell, the hard must-make/must-be-set pruning, the corrected cover-hand trick objective, `solve_moves()` (still refuses), and trick counts generally (still `TRICKS_NOT_COMPUTED` on every same-lean solve) |
| ✅ | ~~Item 60 step 1: two of the same-lean decision procedure's three branches~~ | patch 99 | **The first piece of `solve()` actually computing a same-lean answer, rather than the safe refusal patch 98 shipped.** New `nil/same_lean.hpp`/`.cpp`, wired into `solve()`'s existing same-lean early-return. Handles exactly two of the three branches the full decision procedure needs (see the item 2b entry and the conversation it closes for the proof): neither bid individually cooperatively reachable (`solve_cooperative` on each alone) -- both fail, on cards alone, no search needed at all; and exactly one reachable -- the doomed one fails, and the survivor's fate is settled by an ordinary single-nil check built via `seat_roles_from_nil`, ZERO conjunction machinery touched. The third branch (both individually reachable, needing the conjunction probe and the delicate-cell tiebreak) is explicitly step 2, not guessed at here: `solve()` reports `determined=false` from `solve_same_lean_partial` as refusal, same as before, with a message naming which branch is still missing. **A REAL BUG FOUND WHILE WIRING THIS IN, NOT ASSUMED SAFE.** The single-nil check for the survivor was first invoked the SAME way a hand-checked example was two turns ago; that example's root cause is `nil_oracle.py`'s `_search`, not this codebase, but the caution it taught applied directly here -- built via `seat_roles_from_nil` (full `SeatRoles`, no parity assumption) rather than any shortcut, and checked against `solve_opposing_nils` before trusting it, not after. **CORRECTNESS.** Two hand-picked examples checked directly against the oracle first (an all-hearts 8-card deal and a one-card-each 4-card deal, both already in the test suite for an unrelated reason, both now legitimately DETERMINED instead of refused) -- `nil_makes` matched exactly in both. Then `tools/same_lean_partial_crosscheck.py` (new ctest), which also checks the thing that matters most here: that the undetermined branch REFUSES rather than guesses, since a wrong-looking success there would be a correctness hazard, not a missing feature. 200 deals, both leans: 8 landed in the both-fail cell, 107 in the exactly-one cell, 85 correctly refused as undetermined, zero mismatches. Two pre-existing unit tests that asserted the OLD "same-lean always refuses" behavior were updated to check the new, correct, per-branch behavior instead of being deleted. 33/33; nothing else changed. **NOT IN THIS PATCH:** the third branch (conjunction + delicate cell), the hard must-make/must-be-set pruning, the corrected cover-hand trick objective (verified needed, not yet built -- a direct check against pair-total-plus-tiebreak found real disagreements, e.g. one hand where the existing single-nil-style objective would report the cover hand taking 2 tricks when the true optimum, checked by a dedicated minimax on the cover hand's own count alone, is 1), `solve_moves()` (still refuses outright, unchanged), and trick counts generally (still `TRICKS_NOT_COMPUTED` on every same-lean solve) |
| ✅ | ~~Cooperative reachability probe ported to C++ (item 2b)~~ | patch 98 | **The fifth probe, and a different kind of question from the four that exist: not "can this side force X against any defence" but "does a line producing X exist at all," every seat cooperating, OR at every node, no adversary.** Ported from `nil_oracle.py`'s `solve_cooperative` (patch 90) into two new files, `nil/cooperative.hpp`/`.cpp`, deliberately NOT threaded through `search.cpp`'s `Ctx` the way item 78's conjunction probe is -- conjunction is still a two-sided minimax question wearing a different value function, this one has no minimising side at all, and bolting it onto `Ctx` would mean auditing every field in it to confirm each one is inert rather than writing code where what is NOT there is the whole of the argument. Standalone `--cooperative <seats>` in `nil_cli`; `--no-memo` and `--no-collapse` apply to it, unchanged in meaning. **WHAT PORTS AND WHAT DOES NOT.** `distinct_moves`'s equivalence reduction (rules.hpp) is proved from the rules alone -- follow-suit, the break flag, who beats whom -- and never mentions the objective, so the same swap-symmetry argument carries over unchanged to a reachability question; it is used here, confirmed answer-neutral by the crosscheck (0 of 96 disagreements with `--no-collapse` across the test population). The main table's relative-rank key does NOT port: it compresses a THRESHOLD comparison against a value this probe does not have. The memo is the literal-state key `statekey.hpp`'s own header comment describes the main table as having replaced -- four raw hand masks, trick, leader, broken -- which is exactly `nil_oracle.py`'s own `(hands, leader, trick, spades_broken)`. A faithful port, not a weak one; relative-rank compression for a boolean OR-search is real future work and a different item. **ONE DELIBERATE DEPARTURE FROM A LITERAL PORT.** `--cooperative` skips the blanket shape-acceptance gate (`validate_seat_roles`) every other mode goes through, because this probe reads no seat's role but the ones it is asked to protect and does not need `roles` to name a shape `solve()` accepts -- helping answer positions `solve()` currently refuses, T's `2/2` and `3/3` cells, is the reason it exists. Confirmed `solve()` and `--conjunction` still refuse those shapes exactly as before, unchanged. **CORRECTNESS.** Hand-checked first against the handoff's own 4-card counterexample (`N:A.2.2. KQ...2 2.3.3. .4.4.3`): N alone, E alone and NE together all come back UNREACHABLE, matching "each bid dies on every line, independently of anyone's preference" exactly. Then `tools/cooperative_cpp_crosscheck.py` (oracle against the new C++, ctest #28) -- every one-bid-per-pair role set in BOTH partner leans, including the same-lean cells `solve()` refuses, since this probe answers those too and they are exactly the ones item 60 needs it for. 36 of 36 protect-queries agree with the oracle on a population that is not all one answer (21 reachable, 15 unreachable in the shipped ctest run), collapsing and the memo both confirmed answer-neutral (0 of 36 each). 32/32; every prior test unchanged. **COST AT 13 CARDS, MEASURED RATHER THAN EXTRAPOLATED, PER THE HANDOFF'S OWN WARNING.** Protecting both bidders on every named 13-card deal in the repo: all of `opposed13.txt` (8 deals) and `opposed13_real.txt` (2 deals) are REACHABLE and cheap, and **the hard single deal answers in exactly 56 nodes** -- matching the handoff's own figure precisely. `opposed13_settled.txt`, which patch 91's `[0,3]`-on-every-deal finding never actually covered, is **UNREACHABLE at 12,056,872 nodes, 14.1s wall-clock** -- ~122x the 98,836-node mean of the other eleven, and well provenanced rather than synthetic: the file's own header already explains why (S's KQ sits under W's AJT9, both bids doomed independently). Random sampling of 13-card deals in the cell T's rule reads (both `--conjunction` calls false, filtered with the already-shipped fast C++ probe rather than the oracle) surfaced three more genuine unreachable cases -- 289,963, 4,243,908 and 28,709,576 nodes -- and confirmed real difficulty on BOTH branches there: of 20 draws, more than half did not resolve inside a 10-second cutoff either way, and the reachable cases that did resolve averaged 2,013,170 nodes, higher than the standard corpus's reachable mean, not lower. **A genuine limitation, found by measuring rather than by inspection: the memo has no size cap.** An early uncapped run hit a case whose process died with a non-zero exit and no stderr, consistent with an OOM kill on this machine's 3.9 GiB rather than any answer being wrong -- the 28.7M-node case above completed fine; a harder one right after it did not. **NOT FIXED HERE, and a refinement of what patch 90 anticipated fixing.** That entry warned "the C++ side is not a transcription of a cheap probe; it needs its own cutoff work, and it should be measured before it is trusted" -- measured now, and what is needed is not a cutoff in the alpha-beta sense: there is no adversary to prune against, so every line keeping every protected bid alive genuinely must be tried on a `False` answer, and no move ordering changes that. What the measurement instead points to is a resource BOUND -- a size cap on the memo, or a node budget in `solve_reachable_outcomes`'s own style (patch 91), admitting an exhausted probe as reachable so the band stays sound. `port the probe, no behaviour change` was this patch's scope; the bound is what item 60 or an immediate follow-up needs before this is safe to wire in against untrusted-size positions. **NOT WIRED INTO `solve()` OR THE CASE TABLE ANYWHERE** -- item 60 is what would consult it; this patch ships the probe standing alone, callable and measurable, touching nothing `solve()` already does |
| ✅ | ~~Opposed workloads re-banked at 512 MiB, closing patch 95's gap~~ | patch 97 | **Confirms, does not replace.** Patch 95 doubled the TT default and re-banked `large.txt` full, but the three opposed workloads and the hard single deal are run by hand rather than by a corpus row and were missed -- flagged as the first task before anything else changed. Re-measured against the now-current 512 MiB default: `opposed13.txt` 351,156,828 (was 368,219,325 at 256 MiB), `opposed13_settled.txt` 55,428,602 (was 55,488,773), `opposed13_real.txt` 171,731,064 (was 172,739,931), the hard single deal 241,340,102 (was 258,394,757 -- patch 95's own headline number, and it is `d13-0004` inside `opposed13.txt`, so confirming the total confirms this one too rather than re-checking it separately). **All four match exactly; nothing here was wrong, only unrecorded.** No code changed -- this entry is ROADMAP.md only, and is the one place the 512 MiB figures for these four now live. Historical entries above (patches 88, 89 and 92 among them) keep their 256 MiB figures unchanged, because they record what was measured at the time. 31/31 |
| ✅ | ~~TT default raised 256 -> 512 MiB (item 95)~~ | patch 95 | **6.6% of the nodes on the hard opposed deal for a one-line change, and every rep a win on the clock too.** The two-nil shape was evicting: 2,226,872 evictions against 12,856,780 stores at 256 MiB, 17% of stores displacing a live position, with an already-good 84.5% hit rate. Doubling takes 258,394,757 -> 241,340,102 and won on time in all three interleaved pairs (28.83/28.73/28.46 against 27.86/28.16/27.59). **NOT a "bigger is better" result.** Throughput falls monotonically as the table grows -- 11.3M nodes/sec at 16 MiB down to 7.6M at 2048 -- so the node saving and the cache cost run against each other and the curve has a FLOOR rather than a slope: 256 MiB 28.8s, 512 27.9s, 1024 28.8s, 2048 31.6s. 512 is the bottom, and it is the bottom FOR THIS MACHINE, since the optimum tracks L3 size; a deployment on very different hardware should re-measure rather than inherit the constant. **One banked count re-banked with cause: `large.txt` full 163,393,676 -> 163,149,275** (0.15%, and the only corpus big enough to fill the table at either size). positions.txt fast/full, multinil.txt and large.txt fast are bit-identical -- too small to evict either way. Historical entries above keep the old figure, because they record what was measured at the time. 31/31 |
| ⊘ | Cache-layout work on the TT probe: prefetch, alignment, associativity (item 96) | REJECTED, patch 96 | **All three measured, none worth having, and the hypothesis behind them was wrong.** A bucket is WAYS(4) x sizeof(TTEntry)(24) = **96 bytes and straddles two 64-byte lines**, which looked like an obvious target against 83,041,432 probes on one deal. Measured against a bit-identical control: **prefetching the trailing lines** 28.14s mean against 28.31s, 0.6% and inside the spread on every rep; **alignas(32)** 28.28s, 0.1%, nothing; **WAYS=2**, to fit a bucket in one line, costs 7.7% MORE nodes (258,394,757 -> 278,409,601) for a 5.2% net LOSS, since halved associativity evicts more than the cache saving returns. Prefetch and align were clean experiments -- node counts bit-identical to control, so the timing delta is the entire effect. **THE MECHANISM CLAIM WAS THE ERROR.** "Memory-latency bound" was inferred from throughput falling as the table grows, but a larger table also holds deeper entries, so the nodes that survive are HARDER ones: falling nodes/sec is a composition effect as much as a stall effect, and reading it as latency alone is what motivated all three arms. If probes were really stalling on DRAM, prefetching a straddled bucket would show something; it does not, so the hardware prefetcher and out-of-order execution are evidently already covering it. **What would revive it:** a profile showing actual stall cycles on the probe load, rather than an inference from a throughput curve. `perf` was unavailable in the session that made the inference, which is exactly why it went unchecked |
| ✅ | ~~A 13-card corpus with INDEPENDENT verdicts (item 94)~~ | patch 94 | **`tests/corpus/nil13_verdict.txt`, 400 single-nil 13-card deals contributed by T, verdicts from outside this solver — and all 400 matched on first run.** The repo had no such population: `positions.txt` stops at 6 cards, `large.txt` has 3 rows at 13 and its own header calls them deliberately not average, and `opposed13.txt` is 8 deals selected for being hard. Several measurements have now come back empty on those corpora for selection reasons alone — patch 89's static doom check fired on 0 of 8, patch 91's reachability bound narrowed 0 of 8 — and this is the corpus that can tell an idea that does not work from a corpus that cannot see it. It is what showed item 93 to be redundant rather than merely unhelpful. **Recorded honestly as RANDOM deals, not hands a player would choose to bid**, so any firing rate measured here is an upper bound on a real table: a player looking at a doomed hand does not bid it. The 37 deals whose verdict depends on the opening lead are excluded, since a corpus row pins one leader and those have no single answer to pin; they remain interesting on their own and 36 of 37 break on side lines, with the cover partner's lead never killing the bid. Trick columns are left unrecorded rather than filled from this solver, so a column that has not been independently checked does not look like one that has. 755M nodes in fast mode, so a bench workload and not a ctest fixture |
| ⊘ | Hoist the forced-trump doom certificate to the fast-mode root (item 93) | REJECTED, patch 93 | **Saves 89 nodes out of 755,055,821 — and is slower.** The idea was T's: prove statically that a bid must win a trump trick, and answer fast mode without searching. The proof is real and `cover_deficit_depth` / `nil_must_take_a_trick` in `bounds.hpp` already implements it — **which is the whole result: the solver already had this certificate, already gated correctly, already wired in.** Paired reps on the 400-deal `nil13_verdict.txt`: 755,055,732 nodes with the hoist against 755,055,821 without, i.e. **one node saved per firing**, because the existing machinery already settles those positions in a single node; wall clock was worse in both reps (168s vs 163s), the cost of a `forced_spade_tricks` call on every fast-mode entry. With the existing static proofs DISABLED it saves 5,875,844 nodes (0.7%), which proves the work is real and already being done. **The rediscovered precondition.** The first spelling fired mid-trick and got `c4-0068` wrong — the nil holds a lone ST that the count calls unbeatable while the ACE sits on the table, played by the hand before; `forced_spade_tricks` reads HANDS and a card already played is invisible to it. The existing call site carries the comment *"Only at a trick boundary, which is where the proofs' own preconditions hold"* — the constraint was known and written down, and reimplementing rather than reading rediscovered it as a failing test. **A precision that was a theorem, not a measurement.** Both T and this session quoted "100% precision, zero false positives on 89 deals" as a finding. It is guaranteed a priori: the predicate is a soundness proof, so it CANNOT fire on a makeable deal, and observing that it does not tests only that the port matches. The one informative figure is recall — **89 of 250 doomed deals, 35.6%** — and the other 161 die to placement and tempo, which no per-suit count sees. Rule going forward: where a number is guaranteed by a soundness proof, quote coverage and say which of the two it is. **What would revive it:** a shape the existing static settling does NOT already catch — the hoist would have to answer something the search cannot reach in one node |
| ✅ | ~~Realistic 13-card deals, and the outcome column that was never checked (item 92)~~ | patch 92 | **Two corrections and a corpus.** (1) **Patch 91's cost extrapolation was wrong by ~10 orders of magnitude.** It fitted ~9x per card across 4–6 cards and projected 10^12 at thirteen; the measured cost on the contested corpus is 218 to 20M oracle nodes and **the hardest deal in the repo — the 258M-node one — answers in 56**. The conclusion survives but the reason does not: what kills the deal-specific reach bound is POPULATION, with the band coming back `[0,3]` on 8 of 8 contested deals and on both realistic deals here. The 63–70% narrowing was a 4-to-6-card artifact, where a random hand often holds a forced winner by accident. The broken rule is a recorded one — *measurements are tree-specific* — applied to card count instead of search shape, and the check that falsified it took four seconds on a tool that already existed. (2) **`nils_set` was verified in NEITHER mode on an opposed row.** `nil_bench` checked the outcome column only under MODE_FAST, and fast mode refuses every shape with more than one bidder; full mode checked only the trick columns. With one bidder that was harmless, since `nil_tricks > 0` and `nils_set == 1` say the same thing — with a bid on each side `nil_tricks` is the two bidders' COMBINED count, so "one bid down" and "both down" were indistinguishable to the checker. Found by mutating a corpus row and watching it pass. Full mode now checks it; all existing corpora pass unchanged, so no stale value was being hidden. (3) **`tests/corpus/opposed13_real.txt`** — two 13-card deals contributed by T from the table rather than generated, 172,739,931 nodes. They matter because `opposed13.txt` is eight CONTESTED deals that all end `(fail, fail)`: a presolve whose job is bounding the outcome rank was being measured against a corpus that only ever reaches one rung. These add `(fail, make)` and `(make, make)`, and T stated each outcome before it was run, so the outcome column has an independent witness even though the trick counts are solver-pinned. 31/31; every banked count unmoved |
| ✅ | ~~Which outcomes the CARDS permit: the deal-specific reach bound (item 91)~~ | patch 91 | **Patch 79's bound made deal-specific, and the answer is that it does not scale.** 79 bounds the reachable ranks STRUCTURALLY — the rank is a step function of the broken-bid mask, a bid never un-breaks, so from mask m the reachable ranks are those of the masks containing m. True of every deal and therefore blind to the deal. This asks the cards instead: one existence probe per outcome, four on the opposed shape, and an outcome no line of play reaches cannot be the game value whoever is steering. **Complementary to A/B/C in the direction they cannot see** — those bound the value by what a side can FORCE, this by what the cards PERMIT, and neither implies the other. **Population is excellent: 65% / 63% / 70% of deals at 4 / 5 / 6 cards have at least one outcome blocked**, ~62% get a band narrower than the full ladder, and 2–7% collapse to a single outcome — the handoff's own counterexample `N:A.2.2. KQ...2 2.3.3. .4.4.3` permits ONLY `(fail, fail)`, band `[2,2]`, no primary dimension left to search. **AND THAT IS WHERE IT ENDS — SEE PATCH 92, WHICH CORRECTS THE REASON.** This entry originally claimed the probe cost extrapolates past 10^12 nodes at 13 cards, fitted from 3,402 / 31,332 / 293,702 mean nodes at 4 / 5 / 6. **That extrapolation was wrong by about ten orders of magnitude and should never have been made** — the measured cost on the contested 13-card corpus is 218 to 20M oracle nodes, and the hardest deal in the repo answers in **56**. What actually kills it is POPULATION, not cost: at 13 cards the band comes back `[0,3]` on 8 of 8 contested deals and on the realistic deals of patch 92 as well. The 63–70% narrowing above is a 4-to-6-card artifact, where a random hand often holds a forced winner by accident. **The C++ version must be a STATIC proof of unreachability, not a search for one**, and this patch is its ground truth rather than its prototype; `forced_spade_tricks` in `bounds.hpp` is the existing example of the right shape. Verified four ways: brute-force enumeration per outcome, the game value's own outcome always among the reachable set, agreement with patch 90's cooperative probe where they overlap, and budget-only-widens. A wiring bug was caught by the measurement refusing to move — the budget parameter was threaded through the context and the check but never passed to the constructor, so four caps reported identical numbers; patch 78's gotcha again. 31/31; no C++ touched |
...
**Phase E's doom downgrade: retyping a proven-dead nil from type 1 to type 3
(patch 105).** Measured in Phase A, rejected before implementation. Two
independent measurements kill it and either one would have been enough.

*What it claimed.* The rearchitected pipeline gives full mode three seat types:
0 (must take no tricks), 1 (must take at least one trick), 3 (generic
team-trick maximiser). A nil that is going to be set must be pinned to type 1
so the search cannot wander into lines where it survives — and a type-1 pin
needs a `satisfied` bit in the transposition key, carried for the whole search,
to record whether the seat has taken its trick yet. The proposal was that once
a nil is *proven* dead the pin becomes redundant: retype it to 3, drop the bit,
get the whole search back at full width. The plan put the cost of the pin at "a
wider key and a lower hit rate on every node," which made the downgrade look
like a structural win rather than a micro-optimisation.

*Measurement 1 — it can almost never fire (A4).* A downgrade is licensed only
where the nil is dead on EVERY line, including lines its own side would never
choose. That is what `solve_cooperative` answers. On `nil13_verdict.txt`: 250
of 400 rows have the nil forced set; `nil_must_take_a_trick` settles 89 of them
statically and for free; the remaining 161 were probed at an 8M node budget.
**0 came back unreachable. 146 came back reachable, 15 exhausted.** Total probe
cost 180,785,173 nodes and 106.5 s to learn nothing.

The reason is structural rather than a property of this corpus, and it is worth
stating because it is not the failure mode that was anticipated. A nil forced
set *adversarially* is still usually alive on some line where the opponents
cooperate — that is precisely the difference between "the opponents can force a
trick onto it" and "no line exists." The first is common; the second is rare,
and where it holds the cards usually say so plainly enough that the static
proof already fires. **The probe is not too slow. There is nothing for it to
find that `bounds.hpp` has not already found.**

The anticipated failure was exhaustion — an OR-only search has no cutoff on a
negative answer, so the answer that licenses the downgrade is the expensive one
(patches 98 and 103 both record this). Only 9.3% exhausted. The item dies on
the stronger fact instead.

*Measurement 2 — where it CAN fire is the worst place to do it (A2).* Arm A
`2 3 0 3` (live nil, today's full mode) against arm B `2 3 1 3` (nil declared
down, so primary 0 and the objective becomes plain trick maximisation), same
cards, same leader, one process per solve so no table carries over. Run on all
89 statically-doomed 13-card deals — the exact population the downgrade would
fire on — 88 paired at a uniform 280 s cap:

| | |
|---|---|
| B/A nodes | min 0.26x · p25 1.02x · **median 1.83x** · p75 3.01x · max 10.61x |
| geometric mean / aggregate | 1.74x / **1.77x** (7,212,816,411 → 12,765,796,137) |
| aggregate wall | 856 s → 1474 s = 1.72x |
| throughput | 8.42M vs 8.66M nodes/sec — flat |
| B worse | 66/88 (75%) · ≥2x: 40 (45%) · ≥4x: 14 (16%) |
| B cheaper | 22/88 (25%), best 0.26x |

Throughput being flat matters: the node ratio is the real ratio, so this is not
the win-on-nodes-give-it-back-on-throughput failure that killed items 44, C1
and C3. The one censored deal biases *against* B — `v13-0217` arm A finished in
87.6 s, arm B exceeded 280 s.

*The mechanism, read off the output rather than inferred.* Arm B reports
`presolve_nodes=0`, because `presolve_eligible` is gated on
`!roles.nil_already_set()`, so item 23's root window is simply absent; and
`nil_must_take_a_trick`'s branch in `full_static_bounds` is guarded off because
`per_nil` goes negative once the primary weight is zero. **Plain trick
maximisation runs with almost none of the machinery the nil question gets.**
The downgrade does not remove a constraint, it removes the only structure the
solver currently has for pruning this shape.

*The arithmetic that finishes it.* The bit costs 3.5% (A1, median over 24
paired 13-card deals). Losing the nil constraint costs 83% (A2, median over the
89). The downgrade is available on 0 of 161 deals the static proofs do not
already settle (A4). **Type 1 is therefore not merely the safe default under
the plan's own asymmetry rule — on this corpus it is the faster one.**

*What is a proxy here and what is not.* A2's arm A is today's nil-weighted
lexicographic objective, not the new type 1, which does not exist yet. So the
1.83x is **not** a measurement of "type 1 beats type 3 under the Phase D
engine" and must not be quoted as one. What it measures is the cost of removing
the nil-aware pruning structure from this shape, on the real 13-card trees, on
the real population. That is enough to reject an item whose entire case was
that removing it is free. A1 is a proxy in the same sense and its entry says so.

*A sampling error worth recording, because it nearly set the direction.* The
first deal measured came back at 8.38x and was `v13-0000` — which carries
`nils_set=0`, so the nil MAKES and the deal is not in the downgrade population
at all. A stratified 25-deal sample of genuinely doomed deals then read 1.25x
median (static 1.55x, non-static 0.89x), and the correct 89-deal population
reads 1.83x. Three populations, three answers, same question. The 89 is the
one that decides the design because it is the one the downgrade fires on.

*What would revive it.* Two things, and both are needed rather than either:

1. A static doom proof with materially better recall than
   `nil_must_take_a_trick`'s 89 of 250 — because A4 shows the cooperative probe
   will not supply the difference, and a downgrade nobody can license is worth
   nothing however cheap it is. Note that item 83 already failed on the mirror
   of this: cheap sufficient conditions are not a decision procedure, and the
   common case (a live bidder holding spades WITH adequate covers) gets
   neither proof.
2. A demonstration that type 3 is cheaper than type 1 **under the Phase D
   engine**, where type 1 is a filter rather than a primary weight and the
   presolve window and static proofs have been reformulated for it. A2 measures
   the proxy, not that. If Phase D's type-1 pin turns out to inherit none of
   the pruning arm A gets, the 1.83x does not transfer and this item should be
   re-measured rather than re-rejected on this entry.

*Consequences already taken.* `makes_cooperatively` leaves the pipeline —
answering design question Q3: not on the wire, not computed, not reported.
`solve_cooperative` stays as correct, cross-checked, callable code with its own
ctest; nothing in the presolve consults it. `--no-doom-downgrade` has nothing
left to gate. Phase E reduces to the seat-type translator plus an unconditional
type-1 default for a dead nil. And **item H is re-justified around decision 9
rather than around Phase E**: with the downgrade gone, what still manufactures
`3 3 3 3` in bulk is every mid-hand call on an already-dead nil, and A2
measures that shape at 1.77x aggregate with no bounds at all. H's risk row
should point at decision 9, not at the downgrade.
...
#### The firing rate, measured at patch 86

Item 81 died in the gap between *a claim this weak would suffice* and *a claim
this weak is provable*, so the gap was measured before anything was spent. The
proof used is the cheapest sound one in the file: `top_spade_run`, the holder of
the top outstanding spades and how many it holds consecutively. Spades are
trump, nothing beats them, and each wins the trick it is played on -- a floor on
that seat's tricks, and so on its side's, for one mask test and a short loop.
**Deliberately the weak version**: no side-suit winners, no ruffs, no bound from
the other end.

| | opposed13 | settled deal |
|---|---:|---:|
| already decided by arithmetic | 24.38% | 14.77% |
| **spade run proves the claim** | **12.97%** | **8.10%** |
| **right side, run too short** | **45.83%** | **61.38%** |
| wrong side, or no spades left | 16.72% | 15.61% |

**The middle row is the finding.** In 46% and 61% of the region the top-spade
holder IS the side that has to prove something and the run is simply too short.
The proof is being asked the right question and falling short on STRENGTH --
which is the opposite of item 81, where the proofs missed the case entirely and
there was nowhere to go. Here there is somewhere to go, and it is the rest of DDS
section 3: side-suit winners and ruffs, which the spade run ignores by
construction. Only 16-17% is out of reach of a spade-flavoured proof altogether.

**So the weak proof alone is worth 1.61% of all nodes on `opposed13.txt` and
2.90% on the settled deal** -- real but thin, about item 79's tail rather than
its head. **The case for writing the full bound is the 45.83% / 61.38%, not the
...
### 93. Hoisting the forced-trump doom certificate — ⊘ **REJECTED, patch 93**

T's proposal, and the proof behind it is sound: walk the spade ranks from the
ace down, and if the nil bidder ever holds more spades at or above a rank than
there are higher spades outside to cover them, pigeonhole forces one of them to
win. Sound **for spades specifically**, because a spade is the one card the
bidder cannot discard its way out of — it goes on a spade lead or it ruffs, and
both win.

**The solver already has it.** `cover_deficit_depth` and `nil_must_take_a_trick`
in `bounds.hpp`, gated at the call site by `st.trick_len == 0`. Hoisting it to
the fast-mode root re-answers a question the search already answers.

#### The measurement

Paired reps, `nil13_verdict.txt`, 400 deals, arm toggled at runtime:

| arm | nodes | wall |
|---|---:|---:|
| hoist ON | 755,055,732 | 168.9s / 167.8s |
| hoist OFF | 755,055,821 | 163.4s / 161.8s |

**89 nodes** — exactly one per firing, because the existing static settling
already resolves those positions in a single node. Both reps were *slower* with
it on, which is the `forced_spade_tricks` call charged against every fast-mode
entry.

With the existing proofs disabled the hoist saves 5,875,844 nodes (0.7%), so the
work is real. It is simply already done.

#### The precondition, rediscovered as a failing test

The first spelling fired mid-trick and got `c4-0068` wrong:

```
--pbn 'N:.T..T3 T.2.95. .54.4. .A8..5' --leader S --seats 3 0 3 2
--spades-broken --trick 'S5 S4 SA'
```

The nil holds a lone `ST`. The count calls it unbeatable — and the **ace is on
the table**, played by the hand before. `forced_spade_tricks` reads HANDS, so a
card already played is invisible to it, and the certificate's premise (every
outstanding spade is in somebody's hand) silently fails.

The existing call site already says so: *"Only at a trick boundary, which is
where the proofs' own preconditions hold."* Reimplementing instead of reading
turned a written-down constraint into a debugging session.

#### A precision that was a theorem

The certificate was reported — by T and by this session — as "100% precision,
zero false positives across 89 deals," presented as a finding. **It is
guaranteed before any data is looked at.** The predicate is a soundness proof,
so it cannot fire on a makeable deal; observing that it does not tests only that
one implementation matches another.

The single informative figure is **recall: 89 of 250 doomed deals, 35.6%**. The
other 161 die to placement and tempo, which no per-suit count can see.

The same audit applies in the other direction: `cover_proof` is a *safety*
certificate, so "fires on 0 of 250 doomed" is equally guaranteed, and its
reported ~5% at 8 cards / 0.2% at 13 is the real number.

> **Rule.** Where a figure is guaranteed by a soundness proof, only coverage is
> informative — and the write-up must say which of the two it is quoting.

#### What would revive it

A position shape the existing static settling does *not* already reach in one
node. The hoist has to answer something the search cannot, and on this corpus
...
#### And that is where it ends — but NOT for the reason first recorded

**This section originally claimed the cost extrapolates past 10¹² nodes at
thirteen cards, fitted at ~9x per card from 4 / 5 / 6.  That was wrong by about
ten orders of magnitude.**  See patch 92 for the correction and the measured
numbers; the summary is that the probe is cheap at 13 cards and useless there
...
| 5 | 31,332 |
| 6 | 293,702 |
| **13, contested corpus** | **218 – 20,000,000** |
| **13, the hardest deal in the repo** | **56** |

What kills it is **population**. At thirteen cards the band comes back `[0,3]`
— no narrowing at all — on 8 of 8 contested deals and on the realistic deals
contributed in patch 92. The 63–70% figure above is a small-hand artifact: a
random 4-card hand frequently holds a forced winner by accident, and a real
13-card deal does not.
...
defence*. This one asks something structurally different: **does a line exist at
all**, with all four seats helping rather than any of them opposing? Nothing
above answers it, and a guarantee that a side *cannot* force an outcome says
nothing about whether that outcome is available when nobody is opposing it.

Item 60's case table needs it. The 2/2 rule reaches a cell where neither side
can force anything cleanly and both would rather have both bids live, and names
`(make, make)`. That is a policy, not a derivation.

#### The policy is wrong on the majority of its own cell

| cards | neither-conjunction cell | `(make, make)` UNREACHABLE inside it |
|---:|---:|---:|
| 4 | — | **55%** |
| 5 | 52% | **61%** |
| 6 | 62% | **57%** |
...
The cell is common, and inside it the comfortable outcome usually does not
exist. The handoff carried one counterexample and framed this as a hazard to
guard against; the measurement says the guard is the load-bearing part. Any 2/2
answer produced without this probe is wrong more often than right in the cell
where the rule actually fires.

#### The cost prediction was backwards in the half that matters

The prediction was *"pure existence question — no adversary, OR at every node,
stops at the first success, so it should be very cheap."* True, and only when
the answer is **yes**:

| cards | reachable (mean nodes) | unreachable (mean nodes) | ratio |
|---:|---:|---:|---:|
| 5 | 399 | 12,411 | **31x** |
| 6 | 1,265 | 87,538 | **69x** |
| 7 | 67,218 | 857,951 | **13x** |

An OR-only search has no adversary and therefore no cutoff on a negative answer:
it must exhaust *every* line that keeps both bids alive before it can say none
does. First-success termination is worth a great deal on `True` and exactly
nothing on `False` — and `False` is the answer most of the time in the cell the
probe exists for. **The probe is expensive precisely where it is needed.** The
memo split confirms the mechanism: it is worth 2.46x on reachable and 10.13x on
unreachable, because the exhaustive half is where transpositions recur.

This reshapes item 2b. The C++ side is not a transcription of a cheap probe; it
needs its own cutoff work, and it should be measured before it is trusted.

#### Why the state carries no broken-bid mask

A protected bid survives exactly when its seat wins no trick, so the moment one
does the line is dead and is abandoned *before* recursion. Every node that
reaches the memo therefore shares the same history in the only respect that
matters — no protected bid down — and the key does not have to say so.
`_search_conjunction` needs the mask in its key because a broken bid there
changes a subtree's *value*; here it ends the line. This reads like an omission
and is not, which is why it is the thing brute force is pointed at.

#### Three routes, because there is nothing to compare against

| route | catches | result |
|---|---|---|
| brute-force enumeration | the memo key and the line abandonment above | 48/48 |
| minimax ⟹ reachable | a shared misreading of the question | 14/14 |
| monotonicity in the protected set | holds on every deal, needs no known answer | 96/96 |

Population is asserted in both directions — a run where the probe never sees
both answers fails rather than passes, and so does one where the implication's
premise never arises. Mutation-checked by disabling line abandonment, which
fired all three routes and the population guard.

#### Two fixtures worth keeping

- `N:A.2.2. KQ...2 2.3.3. .4.4.3` — the handoff's counterexample, answered
  UNREACHABLE in 39 nodes. On this deal each bid is doomed even *alone*, which
  is recorded because it is easy to misread it as the joint case.
- `N:4.A.T. .4.A.T .J5.5. J..2.4` — each bid survives alone, **no line carries
  both**. This is the deal proving the question is genuinely joint and does not
  decompose into two single-bid questions; without it the probe could be
  suspected of being redundant against probes A and B.

#### Follow-on, not folded in

The expensive branch wants a cheap sufficient condition for UNREACHABLE.
Monotonicity gives one shape of it — if either bid alone is unreachable then
both together are — but the single-bid question is the same search with a
*smaller* protect mask, so it prunes less and may not be cheaper. Worth
measuring before it is built; it is a separate variable and does not belong in
this patch.
