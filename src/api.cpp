...
std::int32_t prepare(const char* pbn, std::int32_t leader, const char* current_trick,
    ...
    opts.tt_boundaries_only = (flags & NIL_FLAG_TT_ALL_PLIES) == 0;
    opts.target_bounds = (flags & NIL_FLAG_NO_TARGET_BOUNDS) == 0;
    opts.later_tricks = (flags & NIL_FLAG_NO_LATER_TRICKS) == 0;
    opts.spade_matrix = (flags & NIL_FLAG_SPADE_MATRIX) != 0;
